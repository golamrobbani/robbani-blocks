...
(() => {
  ...
  const { editor_type } = rb_style_handler;

  const makeIdOfTemplatePart = (theme, slug) => theme && slug ? theme + "//" + slug : null;
  const emptyFuns = () => { };

  let rbEditGetPreviewDeviceType = false;

  if (editor_type === "edit-site") {
    rbEditGetPreviewDeviceType = select("core/edit-site")
      .__experimentalGetPreviewDeviceType;
  } else if (editor_type === "edit-post") {
    rbEditGetPreviewDeviceType = select("core/edit-post")
      .__experimentalGetPreviewDeviceType;
  }

  window.rbEditCurrentPreviewOption = rbEditGetPreviewDeviceType();
  ...
  const regexForBlockNameTest = /^robbani\-blocks\//g;
  ...
  const generateAndSendStyleToBackend = () => {
        ...
        if (isFunction(isReusableBlock) && isReusableBlock(item)) {
          ...
          if (parsedBlocks) {
            for (const blockItem of parsedBlocks) {
              ...
              if (blockMeta && blockRoot === "robbani_block") {
          ...
          cssObjectMaker(innerBlocks);
        } else if (blockMeta && blockRoot === "robbani_block") {
          styles[blockId] = blockMeta;
        }
        if (innerBlocks.length > 0) {
          cssObjectMaker(innerBlocks);
    ...
    jQuery.ajax({
      ...
      data: {
        ...
        action: "rb_write_block_css",
        nonce: rb_style_handler.sth_nonce,
      },
      error: function (msg) {
        console.log(msg);
      },
  ...
  const callBackFuncOnPreviewChange = () => {
    if (window.rbEditCurrentPreviewOption !== rbEditGetPreviewDeviceType()) {
      const newDeviceType = rbEditGetPreviewDeviceType();

      // the following line of code should be at the top of this if statement
      window.rbEditCurrentPreviewOption = newDeviceType;
